...
export default function VehicleRequestDetailsPage() {
  ...
  const handleReject = async () => {
    ...
    setIsActionLoading(true)
    try {
      await rejectVehicleRequest(request.id, rejectionReason.trim())
      ...
      <Dialog open={rejectDialog} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você pode informar o motivo da reprovação para <strong>{request.brand} {request.model}</strong> (opcional)
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
